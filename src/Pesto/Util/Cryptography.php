...
/**
 * Based on fCryptography functionality, including hashing, symmetric-key encryption and public-key encryption
 *
...
class Cryptography {
	...
	const checkPasswordHash   = 'Cryptography::checkPasswordHash';
	const hashHMAC            = 'Cryptography::hashHMAC';
	const hashPassword        = 'Cryptography::hashPassword';
	const publicKeyDecrypt    = 'Cryptography::publicKeyDecrypt';
	const publicKeyEncrypt    = 'Cryptography::publicKeyEncrypt';
	const publicKeySign       = 'Cryptography::publicKeySign';
	const publicKeyVerify     = 'Cryptography::publicKeyVerify';
	const random              = 'Cryptography::random';
	 ...
	 *
	 * @throws Exception  When the private key is invalid
	 *
	...
	static private function hashWithSalt($source, $salt)
	{
		...
		return 'Cryptography::password_hash#' . $salt . '#' . $sha1;
	}
	 ...
	 *
	 * @throws Exception  When the ciphertext appears to be corrupted
	 *
	...
	static public function publicKeyDecrypt($ciphertext, $private_key_file, $password)
	{
		...
		if (sizeof($elements) != 4 || $elements[0] != 'Cryptography::public') {
			throw new Exception(
	...
	static public function publicKeyEncrypt($plaintext, $public_key_file)
	{
		...
		return 'Cryptography::public#' . base64_encode($encrypted_keys[0]) . '#' . base64_encode($ciphertext) . '#' . $hmac;
	}
	...
	static private function seedRandom()
	{
		...
		if (!(strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') && $handle = fopen('/dev/urandom', 'rb')) {
			$bytes = fread($handle, 4);
			fclose($handle);

		// On windows we should be able to use the Cryptographic Application Programming Interface COM object
		} elseif ((strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') && class_exists('COM', FALSE)) {
			try {
	...
	static public function symmetricKeyDecrypt($ciphertext, $secret_key)
	{
		...
		// We need to make sure this ciphertext came from here, otherwise we are gonna have issues decrypting it
		if (sizeof($elements) != 4 || $elements[0] != 'Cryptography::symmetric') {
			throw new Exception(
				'The ciphertext provided does not appear to have been encrypted using %s',
	 ...
	 * encryption and decryption.
	 *
	 * @throws Exception  When the $secret_key is less than 8 characters long
	 *
	 * @param  string $plaintext   The content to be encrypted
	...
	static public function symmetricKeyEncrypt($plaintext, $secret_key)
	{
		...
		return 'Cryptography::symmetric#' . $encoded_iv . '#' . $encoded_ciphertext . '#' . $hmac;
	}
